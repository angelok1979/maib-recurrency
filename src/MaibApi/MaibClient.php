...
class MaibClient extends GuzzleClient
{
    ...
    /**
     * Registering new regular template for future regular payments
     * @param  int $currency
     * @param  string $clientIpAddr
     * @param  string $language
     * start registration. 
     ...
     * error          - in case of an error
     */
    public function registerRegularTransaction($currency, $clientIpAddr, $biller_client_id, $language = 'ru', $owerwrite = null)
    {
        $args = [
          'command'  => 'p',
          'currency' => (string)$currency,
		  ...
		  'perspayee_expiry' => (string)(date('my', strtotime('+5 years'))),
		  'perspayee_gen' =>   1,
		  'perspayee_overwrite' => isset($owerwrite) ? '1' : '0'  ,  
          'language' => $language
        ];
	
        return parent::registerRegularTransaction($args);
    }

   /**
     * Make transactions with regular template for payments
     * @param  string $amount          		
     * @param  int $currency
     * @param  string $clientIpAddr
     * @param  string $language
     ...
     * error          - in case of an error
     */
    public function makeRegularTransaction($amount, $currency, $clientIpAddr, $merch_id, $language = 'ru')
    {
        $args = [
          'command'  => 'e',
          'amount' => (string)($amount * 100),
          ...
          'client_ip_addr' => $clientIpAddr,
          'language' => $language,
		  'biller_client_id' => $merch_id 
        ];
        return parent::makeRegularTransaction($args);
    }

   /**
     * Delete regular template for payments
     * @param  int $merch_id
     * remove registration
     * @return  OK
     * error          - in case of an error
     */
    public function deleteRegularTransaction($merch_id)
    {
        $args = [
          'command'  => 'x',	 
		  'biller_client_id' => $merch_id        
        ];
        return parent::deleteRegularTransaction($args);
    }
